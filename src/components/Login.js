 ...
 className="absolute text-white p-12 w-4/12 my-24  mx-auto left-0 right-0 bg-black bg-opacity-90 ">
        ...
        {!isSignInForm &&
          <input ref = {fullName} type="text" placeholder="    Full Name"
           className="py-3 my-3  w-full bg-gray-800 rounded-lg" />
        }
        <input ref = {email} type="text" placeholder="    Email or Phone number" 
        className="py-3 my-3  w-full bg-gray-800 rounded-lg" />
        <input ref = {password} type="text" placeholder="    Password" 
        className="py-3 my-2  w-full bg-gray-800 rounded-lg" />
        ...
        <div className="flex cursor-pointer" onClick={toggleSignInForm}>
          <p className="py-3 text-gray-400 ">{isSignInForm ? "New to Netflix?" : "Already registered?"}</p>
